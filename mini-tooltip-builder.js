...
class MiniTooltipBuilder extends HTMLElement {
  constructor() {
    ...
    s.textContent = `
      :host { display:block; font: 12px/1.4 sans-serif; padding:8px; }
  ...
  onCustomWidgetAfterUpdate(settings) {
    (settings && this._fields).forEach(id => {
      const el = this.shadowRoot.getElementById(id);
      if (el && settings[id] !== undefined && settings[id] !== null) {
  ...
  getAddOnSettings() {
    ...
    this._fields.forEach(id => {
      const el = this.shadowRoot.getElementById(id);
      if (!el) return;
      out[id] = (id === 'maxPoints') ? Number(el.value || 10) : el.value;
    });
    return out;
  }
}
